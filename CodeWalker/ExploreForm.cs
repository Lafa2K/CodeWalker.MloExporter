...
namespace CodeWalker
{
    public partial class ExploreForm : Form
    {
        ...
        private sealed class YtypPropExportTarget
        {
        ...
        private sealed class YtypPropExportResult
        {
        ...
        private sealed class YtypPropExportProgress
        {
        ...
        private bool CanExportAllProps(MainListItem item)
        {
            if (item == null) return false;
            if (item.Folder != null) return false;

            var name = item.File?.NameLower ?? item.Name?.ToLowerInvariant();
            if (string.IsNullOrEmpty(name)) return false;

            return name.EndsWith(".ytyp");
        }
        ...
        private void ShowListContextMenu(MainListItem item)
        {
            ...
            if (item != null)
            {
                ...
                canexportallprops = CanExportAllProps(item);
                canedit = EditMode && !issearch;
            ...
            ListContextExportAllPropsMenu.Enabled = canexportallprops;
            ListContextExportAllPropsMenu.Visible = canexportallprops;
        ...
        private MainListItem GetSelectedFileItem()
        {
            if (MainListView.SelectedIndices.Count == 1)
            {
                var idx = MainListView.SelectedIndices[0];
                if ((idx >= 0) && (idx < (CurrentFiles?.Count ?? 0)))
                {
                    var file = CurrentFiles[idx];
                    if (file?.Folder == null)
                    {
            ...
            return MainListView.FocusedItem?.Tag as MainListItem;
        }
        private string GetExportAllPropsFolderName(MainListItem item)
        {
            var name = item?.Name ?? item?.File?.Name ?? "export";
            ...
            var clean = new string(name.Select(ch => InvalidFileNameChars.Contains(ch) ? '_' : ch).ToArray()).Trim();
            return string.IsNullOrWhiteSpace(clean) ? "export" : clean;
        }
        private async Task ExportAllProps()
        {
            var item = GetSelectedFileItem();
            if ((item == null) || !CanExportAllProps(item))
            {
                return;
            }

            var includeTexturesDialog = MessageBox.Show(this,
                "Do you want to export all related textures too?\n\nYes: export XML files and all related/shared textures.\nNo: export only the XML files.\nCancel: stop.",
                "Export All Props",
                MessageBoxButtons.YesNoCancel,
                MessageBoxIcon.Question);

            if (includeTexturesDialog == DialogResult.Cancel)
            {
                return;
            }
            ...
            var outputFolder = Path.Combine(rootFolder, GetExportAllPropsFolderName(item));
            ...
            Form progressForm = new Form()
            {
                ...
                Text = "Export All Props - CodeWalker by dexyfex",
            ...
            {
                progressForm.Controls.Add(control);
            }
            var titleLabel = new Label() { Left = 20, Top = 18, Width = 480, Height = 20, Text = "Exporting props from " + (item.File?.Name ?? item.Name ?? "YTYP") };
            var countLabel = new Label() { Left = 20, Top = 48, Width = 480, Height = 20, Text = "Preparing..." };
            var statusLabel = new Label() { Left = 20, Top = 72, Width = 480, Height = 20, Text = "Starting..." };
            var progressBar = new ProgressBar() { Left = 20, Top = 104, Width = 480, Height = 22, Minimum = 0, Maximum = 1000, Style = ProgressBarStyle.Marquee, MarqueeAnimationSpeed = 30 };
            AddControl(titleLabel);
            AddControl(countLabel);
            AddControl(statusLabel);
            AddControl(progressBar);

            void UpdateExportProgress(YtypPropExportProgress progress)
            {
                if ((progress == null) || progressForm.IsDisposed)
                {
                    return;
                }
                ...
                try
                {
                    ...
                    {
                        countLabel.Text = (progress.Total > 0) ? (progress.Current.ToString() + "/" + progress.Total.ToString()) : "Preparing...";
            ...
            progressForm.Shown += async (s, e) =>
            {
                try
                {
                    result = await Task.Run(() => ExportYtypProps(item, outputFolder, includeTexturesDialog == DialogResult.Yes, UpdateExportProgress));
                }
                catch (Exception ex)
                {
                    exportException = ex;
            ...
            progressForm.FormClosing += (s, e) => { e.Cancel = inProgress; };
            ...
            {
                MessageBox.Show(this, "Error exporting props:\n" + exportException, "Export All Props", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            ...
            summary.AppendLine(result.ExportedTargets.ToString() + " of " + result.TotalTargets.ToString() + " prop files exported.");
            if (result.ExportedTextures > 0)
            {
                summary.AppendLine(result.ExportedTextures.ToString() + " textures exported.");
            }
            if (result.MissingTextures > 0)
            {
                summary.AppendLine(result.MissingTextures.ToString() + " referenced textures were not found.");
            }
            if (result.MissingArchetypes > 0)
            {
                summary.AppendLine(result.MissingArchetypes.ToString() + " prop archetypes could not be resolved.");
            }
            if (result.MissingResources > 0)
            {
                summary.AppendLine(result.MissingResources.ToString() + " prop resources were not found.");
            }
            if (result.Errors.Count > 0)
            {
                ...
                {
                    summary.AppendLine("...and " + (result.Errors.Count - 20).ToString() + " more.");
                }
            }

            MessageBox.Show(this, summary.ToString(), "Export All Props");
        }
        private YtypPropExportResult ExportYtypProps(MainListItem item, string outputFolder, bool includeTextures, Action<YtypPropExportProgress> progress = null)
        {
            Directory.CreateDirectory(outputFolder);

            ReportYtypPropExportProgress(progress, "Waiting for file cache...");
            var fileCache = WaitForFileCacheReady();
            if (fileCache == null)
            {
                throw new Exception("Game file cache isn't ready yet. Please wait for the explorer to finish loading and try again.");
            }

            ReportYtypPropExportProgress(progress, "Loading YTYP...");
            var ytyp = LoadYtypForExport(item);
            ...
            var targets = new Dictionary<string, YtypPropExportTarget>(StringComparer.InvariantCultureIgnoreCase);
            ...
            ReportYtypPropExportProgress(progress, "Found " + targets.Count.ToString() + " prop files to export.", 0, targets.Count);
            ...
            foreach (var target in targets.Values.OrderBy(t => t.Entry?.Name, StringComparer.InvariantCultureIgnoreCase))
            {
                ...
                ReportYtypPropExportProgress(progress, "Exporting " + current.ToString() + "/" + targets.Count.ToString() + ": " + entry.Name + "...", current, targets.Count);
                ...
                try
                {
                    ...
                    var xml = MetaXml.GetXml(entry, data, out var newFileName, outputFolder);
                    ...
                    File.WriteAllText(Path.Combine(outputFolder, newFileName), xml);
                    result.ExportedTargets++;

                    if (includeTextures)
                    {
                        ExportTargetTextures(fileCache, target, outputFolder, result);
                    }
                }
                catch (Exception ex)
                {
            ...
            ReportYtypPropExportProgress(progress, "Export complete. " + result.ExportedTargets.ToString() + "/" + result.TotalTargets.ToString() + " prop files exported.", result.TotalTargets, result.TotalTargets);

            return result;
        }
        private void ReportYtypPropExportProgress(Action<YtypPropExportProgress> progress, string status, int current = 0, int total = 0)
        {
            UpdateStatus(status);
        ...
        private GameFileCache WaitForFileCacheReady(int timeoutMs = 60000)
        {
        ...
        private YtypFile LoadYtypForExport(MainListItem item)
        {
            var data = GetFileData(item);
            if (data == null)
            {
                throw new Exception("Unable to extract the selected YTYP.");
            }

            var entry = item.File;
            if (entry == null)
            {
                var name = new FileInfo(item.FullPath).Name;
                entry = CreateFileEntry(name, item.FullPath, ref data);
            }
        ...
        private void AddYtypPropTargets(IEnumerable<MCEntityDef> entities, Dictionary<uint, Archetype> localArchetypes, GameFileCache fileCache, Dictionary<string, YtypPropExportTarget> targets, YtypPropExportResult result)
        {
            if (entities == null)
            {
                return;
            ...
            foreach (var entity in entities)
            {
                ...
                {
                    target = new YtypPropExportTarget { Entry = entry };
                    targets[key] = target;
                }
        ...
        private RpfFileEntry GetExportEntryForArchetype(GameFileCache fileCache, Archetype archetype, uint fallbackHash = 0)
        {
            if (archetype != null)
            {
        ...
        private void AddTextureDictHashes(YtypPropExportTarget target, Archetype archetype, uint fallbackHash)
        {
            if (target == null)
            {
                return;
        ...
        private void ExportTargetTextures(GameFileCache fileCache, YtypPropExportTarget target, string outputFolder, YtypPropExportResult result)
        {
            ...
            var textureFolder = Path.Combine(outputFolder, Path.GetFileNameWithoutExtension(target.Entry.Name));
            ...
            CollectTexturesForFile(fileCache, exportFile, target.TextureDictHashes, textures, missingTextures);
        ...
        private void CollectTexturesForFile(GameFileCache fileCache, GameFile exportFile, IEnumerable<uint> textureDictHashes, Dictionary<string, Texture> textures, HashSet<string> missingTextures)
        {
            if (exportFile is YdrFile ydr)
            {
                CollectTexturesForDrawable(fileCache, textureDictHashes, ydr.Drawable, textures, missingTextures);
                return;
            }
            ...
            if (exportFile is YddFile ydd)
            {
                if (ydd.Drawables != null)
                {
                    ...
                    {
                        CollectTexturesForDrawable(fileCache, textureDictHashes, drawable, textures, missingTextures);
                    }
                }
                return;
            }
            ...
            if (exportFile is YftFile yft)
            {
                ...
                CollectTexturesForDrawable(fileCache, textureDictHashes, fragment.Drawable, textures, missingTextures);
                CollectTexturesForDrawable(fileCache, textureDictHashes, fragment.DrawableCloth, textures, missingTextures);
                ...
                if (fragment.Cloths?.data_items != null)
                {
                    ...
                    {
                        CollectTexturesForDrawable(fileCache, textureDictHashes, cloth?.Drawable, textures, missingTextures);
                    }
                }
                ...
                if (childDrawables != null)
                {
                    ...
                    {
                        CollectTexturesForDrawable(fileCache, textureDictHashes, child?.Drawable1, textures, missingTextures);
                        CollectTexturesForDrawable(fileCache, textureDictHashes, child?.Drawable2, textures, missingTextures);
                    }
                }
            }
        }
        private void CollectTexturesForDrawable(GameFileCache fileCache, IEnumerable<uint> textureDictHashes, DrawableBase drawable, Dictionary<string, Texture> textures, HashSet<string> missingTextures)
        {
            if (drawable == null)
            {
                return;
            ...
            foreach (var shader in drawable.ShaderGroup.Shaders.data_items)
            {
                ...
                foreach (var parameter in parameters)
                {
                    ...
                    var resolvedTexture = ResolveTextureReference(fileCache, textureRef.NameHash, textureDictHashes);
        ...
        private Texture TryGetTextureFromYtd(GameFileCache fileCache, uint textureHash, uint textureDictHash)
        {
            ...
            return ytd.Loaded ? ytd.TextureDict?.Lookup(textureHash) : null;
        }
        ...
        private async void ListContextExportAllPropsMenu_Click(object sender, EventArgs e)
        {
            await ExportAllProps();
        }
